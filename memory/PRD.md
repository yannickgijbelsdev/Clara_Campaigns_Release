...
## Changelog — 2026-06 (Iteration 24) — Hide categories from subscribe form
...
## Changelog — 2026-06 (Iteration 26) — Extensive workspace analytics + calendar
- Backend: send_events now record send_type ("all"/"category"/"contacts"/"test"), category_ids, recipients; campaigns store send_type/send_categories. Test sends now logged as send_type "test" (excluded from quota via {"$ne":"test"} filter, missing field still counts historic). NOTE: send_type only exists for sends made from now on; older = "other/legacy".
- New GET /api/analytics?from=&to=&category= returns: totals (campaigns_sent, emails_sent, opened, clicked, failed, test_sends, open/click_rate, subscribers_added, unsubscribed), timeseries (per-day sent/opened/clicked), send_types breakdown, subscriber_growth (per-day added/unsubscribed/net), per_campaign (open/click rates), and by_day map (for calendar heatmap + day detail with campaign list). Category filter narrows delivery/growth metrics to contacts in that tag.
